    ...
    <div className="flex flex-col min-h-full">
      ...
      <div className="sticky top-0 z-10 bg-[#0a0a0a] border-b border-[#1a1a1a] px-4 pt-4 pb-3">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h1 className="text-base font-black tracking-widest text-[#FFB81C]">BRAIN ANALYTICS</h1>
            <div className="text-[8px] text-[#6b6860]">Win rates · P&L per strategy · ROI · Best plays</div>
          </div>
        ...
        <div className="flex gap-1 mb-3">
          {[
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              ...
              style={{
                background: activeTab === tab.id ? '#FFB81C15' : 'transparent',
                color: activeTab === tab.id ? '#FFB81C' : '#444',
                border: `1px solid ${activeTab === tab.id ? '#FFB81C40' : '#1a1a1a'}`,
              }}>
              {tab.label}
            </button>
          ))}
          ...
          <div className="flex gap-1">
            {SORT_OPTIONS.map(s => (
              <button key={s.id} onClick={() => setSortBy(s.id)}
                ...
                style={{
                  background: sortBy === s.id ? '#FFB81C10' : 'transparent',
                  color: sortBy === s.id ? '#FFB81C' : '#333',
                  border: `1px solid ${sortBy === s.id ? '#FFB81C30' : '#111'}`,
                }}>
                {s.label}
              </button>
            ))}
      ...
      <div className="px-4 pt-3 pb-6 space-y-3">
        {isLoading && (
          <div className="text-center py-12 text-[#6b6860] text-[10px] tracking-widest">COMPUTING ANALYTICS...</div>
        )}
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-xl p-3 col-span-2">
                <div className="text-[7px] text-[#3a3a3a] tracking-widest mb-0.5">HIVE REALIZED P&L</div>
                <div className="mono text-xl font-black" style={{ color: totalHivePnl >= 0 ? '#22c55e' : '#ef4444' }}>
                  {totalHivePnl >= 0 ? '+' : ''}${totalHivePnl.toFixed(2)}
                </div>
                <div className="text-[7px] text-[#4a4a44] mt-0.5">from {trades.filter(t => t.status === 'closed').length} closed trades</div>
              </div>
              <div className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-xl p-3">
                <div className="text-[7px] text-[#3a3a3a] tracking-widest mb-0.5">TOP BRAIN</div>
                {bestBrain && (
                  <>
                    ...
                    <div className="mono text-[7px] text-[#22c55e]">+{bestBrain.roi.toFixed(1)}% ROI</div>
                  </>
                )}
              </div>
            </div>
