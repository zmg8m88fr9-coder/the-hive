...
const ALGORITHMS = [
  ...
  },
  {
    ...
    rules: [
      ...
      'Estimate permanent market impact: I_perm = g(order_size / ADV, market_depth)',
      'Model impact decay function over time',
      'Optimize execution schedule to balance immediacy vs. impact cost',
      'Split large orders into smaller child orders',
      'Adjust participation rate based on volatility and spreads',
      'Monitor realized impact vs. pre-trade estimate',
      'Adapt strategy if market conditions shift',
    ],
    bestFor: ['Large Orders', 'Institutional'],
    riskLevel: 'Low-Medium',
  ...
  },
  {
    ...
    rules: [
      ...
      'Estimate volume distribution across time periods',
      'Adjust execution pace relative to expected volume curve',
      'If trading above VWAP trend: be more aggressive early, slower late',
      'If trading below VWAP trend: be slower early, more aggressive late',
      'Monitor cumulative volume and adjust participation rate',
      'Use child orders to blend into natural volume',
      'Exit or adjust if price diverges significantly from VWAP',
    ],
    bestFor: ['All Assets', 'Institutional'],
    riskLevel: 'Low',
    holdTime: 'Minutes to Hours',
  },
  {
    ...
    category: 'Execution',
    color: '#06b6d4',
    description: 'Minimize total cost vs. decision price using Perold framework',
    rules: [
      'Set decision benchmark: price at which order decision was made',
      'Split execution cost into trading cost and opportunity cost',
      'Trading cost = realized price vs. midpoint at decision time',
      'Opportunity cost = unrealized profit/loss on unexecuted portion',
      'Total shortfall = (realized price - decision price) × shares',
      'Optimize execution speed vs. impact cost',
      'Use pretrade analysis to forecast opportunity cost',
      'Post-trade: measure actual vs. expected shortfall to improve models',
    ],
    bestFor: ['All Assets', 'Institutional'],
    riskLevel: 'Low',
    holdTime: 'Minutes to Hours',
  },
  {
    ...
    rules: [
      ...
      'Fit using historical data and non-parametric tests',
      'Calculate residuals: actual_return - predicted_return',
      'Identify assets with significant positive/negative residuals',
      'Long residual > +2σ, short residual < -2σ',
      'Use proper statistical tests (Wilcoxon, Mann-Whitney) to validate',
      'Hedge factor exposures to isolate alpha',
      'Exit when residual mean-reverts or model fit deteriorates',
    ],
    bestFor: ['Correlated Assets'],
    riskLevel: 'Medium',
    holdTime: 'Hours to Days',
  },
  {
    ...
    rules: [
      ...
      'On 5-minute chart: plot the 200 EMA — this becomes your intraday support level',
      'Entry trigger: price breaks AND holds above the daily 9 EMA at the same time the 5-min 200 EMA aligns below price as support',
      'Best setup: stock runs up strongly, then pulls back cleanly to the 5-min 200 EMA without breaking it',
      'Bouncing off the 5-min 200 EMA while holding above the daily 9 EMA = HIGH CONVICTION LONG',
      'Volume should drop on the pullback and spike on the bounce — confirms institutional support at the 200 EMA',
      'Entry: on the first green candle reclaiming the 5-min 200 EMA after a pullback',
      'Stop loss: close below the 5-min 200 EMA (candle close, not just wick)',
      'If price breaks the 5-min 200 EMA decisively with volume: do NOT enter, setup is invalidated',
      'Take profit: prior high of day, next resistance level, or trail stop under 9 EMA on 5-min chart',
      'Avoid: if the daily 9 EMA and 5-min 200 EMA are far apart — confluence requires them to be in close proximity',
    ],
    bestFor: ['Stocks', 'Intraday', 'Momentum'],
    riskLevel: 'Medium',
    holdTime: '30m - 3h',
  },
  {
    ...
    rules: [
      ...
      'Estimate fill probability based on historical order book depth',
      'If probability < 30%: adjust order to higher participation rate',
      'If probability > 70%: keep limit order and wait for passive fill',
      'Monitor queue position and update if price moves',
      'Cancel and replace if opportunity cost exceeds expected savings',
      'Use prediction: demand = a₀ + a₁·(spread) + a₂·(volume)',
      'Track actual vs. predicted fill rates to refine model',
    ],
    bestFor: ['All Assets'],
    riskLevel: 'Low-Medium',
